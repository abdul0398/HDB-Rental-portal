  ...
  const Row: React.FC<ListChildComponentProps> = ({ index, style }) => {
    ...
    return (
      <div
        ...
        } "h-14 grid gap-1 grid-cols-[20%_25%_10%_10%_20%_10%] border-b transition duration-300 ease-in-out hover:bg-neutral-100 dark:border-neutral-500 dark:hover:bg-neutral-600" `}
      >
  ...
  const handleSort = (field: string) => {
    const isSortedAscending = transactions.every((transaction, index) => {
      ...
      if (field === "monthly_rent") {
        return (
          parseInt(transaction[field]) >=
          parseInt(transactions[index - 1][field])
        );
      } else {
          ...
          <div className="overflow-hidden">
            <div className="min-w-full text-left text-sm font-light overflow-hidden">
              <div className="border-b font-medium dark:border-neutral-500 grid gap-1 grid-cols-[20%_25%_10%_10%_20%_10%] text-sm">
                <div className="px-1 py-4 text-sm flex">
                  TOWN{" "}
                ...
                <div className="px-1 py-4 text-sm flex">
                  ...
                  <span className="flex items-center">
                    <FaSort
                      className="hover:cursor-pointer"
                      onClick={() => handleSort("rent_approval_date")}
                    />
                  </span>
                </div>
                <div className="px-1 py-4 text-sm flex">
